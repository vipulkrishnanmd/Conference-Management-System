...
<form id = "autoFindRev" action="Display" method="post">
	
      <input type="hidden" name="fun" value="autoFindRev">
    <input type="hidden" name="pid" value="<%=(Integer) request.getAttribute("pid")%>">
	<input type="hidden" name="cid" value="<%=(Integer) request.getAttribute("cid")%>">
      <input type=submit value="Auto find Suitable Reviewers">
      </form>
 </div>
<br><br><br><br><br>
...
<thead>
	...
	<tbody><tr>
<% Paper thisPaper = (Paper) request.getAttribute("thisPaper");
...
for(User user:users){
	...
	<td>
	<% 	int flag = 0;%>
	<% 	for(User u:thisPaper.getReviewers()){
			if(u.getId() == user.getId()){
				System.out.println("jspthree");
				flag = 1;
			}
		}%>
	<% if(flag == 0){ %>
	<font color=black>
	<form id = "one" action="Display" method="post">
	
	<input type="hidden" name="uid" value="<%=user.getId()%>">
      <input type="hidden" name="fun" value="assignThisUser">
    <input type="hidden" name="pid" value="<%=(Integer) request.getAttribute("pid")%>">
	<input type="hidden" name="cid" value="<%=(Integer) request.getAttribute("cid")%>">
